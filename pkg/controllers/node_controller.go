			...
			case event, ok := <-rc:
				...
				case watch.Added, watch.Modified:
					...
					if c.needHeartbeat(node) {
						...
						c.logger.Printf("set nodeIP: %s", node.Name)
				...
				case watch.Deleted:
					...
					if c.nodesSets.Has(node.Name) {
						...
						c.logger.Printf("unset nodeIP: %s", node.Name)
...
func (c *NodeController) configureNode(node *corev1.Node) ([]byte, error) {
	...
	// update nodeIP
	c.logger.Printf("set nodeIP: %s=%s", node.Name, node.Status.Addresses[0].Address)
	c.nodeIPStatus.SetCache(node.Name, node.Status.Addresses[0].Address)
