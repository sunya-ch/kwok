...
func NewPodController(conf PodControllerConfig) (*PodController, error) {
	...
	n := &PodController{
		...
		nodeIPStatus:                          conf.NodeIPStatus,
	}
...
func (c *PodController) LockPod(ctx context.Context, pod *corev1.Pod) error {
	...
	if err != nil {
		c.logger.Printf("patch error: %v", err)
		return err
	}
	if patch == nil {
	...
	if err != nil {
		if errors.IsNotFound(err) {
			c.logger.Printf("notFound error: %s", pod.Name)
			return nil
		}
		c.logger.Printf("patch error: %s %v", pod.Name, err)
		return err
	}
...
func (c *PodController) configurePod(pod *corev1.Pod) ([]byte, error) {
	...
	if hostIP != nil {
		c.logger.Printf("read hostIP: %s=%s", pod.Spec.NodeName, hostIP)
		...
		errMsg := fmt.Sprintf("no available hostIP for %s on %s", pod.Name, pod.Spec.NodeName)
	...
	c.logger.Printf("Patch %s: %s", pod.Name, string(patch))
